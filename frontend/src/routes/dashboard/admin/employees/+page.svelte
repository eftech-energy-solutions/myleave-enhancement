  ...
  `${PUBLIC_VITE_API_BASE}/api/employee`,
  {
    ...
    body: JSON.stringify({
      ...
      if (res.ok) {
        ...
        addModalOpen = false;
         showToast(
          `Employee ${newEmp.name} added successfully.`,
          "success"
        );
      } else {
        showToast(
          data.error || "Failed to add new employee.",
          "error"
  );
      }
    } catch (err) {
  ...
  async function toggleEditSave() {
    ...
    if (editMode) {
      ...
      try {
        ...
        await loadEmployees();
        showToast(
          `Employee ${detailsForm.name} updated successfully.`,
          "success"
        );
        editMode = false;
        detailsOpen = false;
...
<style>
  ...
  .add-employee-btn{ display:inline-flex; align-items:center; gap:8px; padding:.6rem 1.15rem; border-radius:9999px; font-size:14px; box-shadow:0 6px 14px rgba(0,0,0,.18); white-space:nowrap; }
