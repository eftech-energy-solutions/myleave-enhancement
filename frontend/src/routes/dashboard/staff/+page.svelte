<script>
	...
	$: {
		...
		const validCF = expiry && today > expiry ? 0 : Number(user?.carry_forward_balance ?? 0);
	...
	onMount(async () => {
		try {
			...
			console.log(
				'HOSP DATA — entitlement:',
				user?.hosp_entitlement,
				'balance:',
				user?.hosp_balance
			);
		...
		? [
				{
					...
					remaining: (() => {
						...
						// validCF = 0 once the carry-forward expires (or is fully consumed)
						const cfRemaining = Number(user.carry_forward_balance ?? 0);
						const validCF = expiry && today > expiry ? 0 : cfRemaining;

						// ✅ TRUE remaining = stored AL balance (already expiry-aware in remaining_leave)
						//   + still-valid carry forward = DONUT ALIGNMENT (expired CF → 0, no leak)
						const storedAL = Number(user.leave_entitlement_annual ?? 0);
						return Math.max(0, Number((user.remaining_leave ?? storedAL) - validCF)) + validCF;
					})()
				},
				{
					title: 'Medical Leave Summary',
					total: Number(
						user.leave_entitlement_medical_original ?? user.leave_entitlement_medical ?? 14
					),
					spent: Number(approvedMC || 0),
					pending: Number(pendingMC || 0), // ✅ ADD THIS
					remaining: Number(
						(user.leave_entitlement_medical_original ?? 14) - (approvedMC || 0) - (pendingMC || 0)
					)
				},
				{
					...
					remaining: Number(
						(user.hosp_entitlement ?? user.hosp_entitlement_original ?? 60) -
							(approvedHOSP || 0) -
							(pendingHOSP || 0)
					)
				}
	...
	async function loadHolidays() {
		...
		error = '';
		try {
			const res = await fetch(`${PUBLIC_VITE_API_BASE}/api/holidays`, { credentials: 'include' });
	...
	async function submitLeave(e) {
		...
		if (overlapping.length > 0) {
			...
			showToast(`You have already applied leave on:\n${dates}`, 'warning', 'Overlapping Leave');
			return;
		}
		...
		if (isTechnicalConsultant && leaveType === 'AL' && availableAnnualLeave <= 0) {
			showToast(
		...
		if (leaveType === 'AL') {
			...
			const today = new Date();
			const validCF = cfExpiry && today > cfExpiry ? 0 : cfBalance;

			// Current annual balance already excludes approved leave.
			// Only pending leave must be deducted here.
			availableBalance = annualBalance + validCF - Number(pendingAL || 0);
		} else if (leaveType === 'MC') {
			availableBalance = Number(user.leave_entitlement_medical ?? 0) - Number(pendingMC || 0);
		} else if (leaveType === 'HOSP') {
			availableBalance =
				Number(user.hosp_balance ?? user.hosp_entitlement ?? 0) - Number(pendingHOSP || 0);
		...
		if (availableBalance !== Infinity && Number(totalDays) > availableBalance) {
			showToast(
		...
		try {
			...
			if (!res.ok) {
				...
				if (data?.message) {
					showToast(data.message, 'error', 'Submission Failed');
				} else {
					showToast(
			...
			showToast('Your leave application has been successfully submitted!', 'success');
			await loadRecent();
			await loadAppliedLeave();
			buildMonth(viewBase);
			...
			showToast('Something went wrong while submitting your leave.', 'error');
		}
	}
		...
		try {
			...
			const active = all.filter(
				(r) =>
					...
					(r.status === 'approved' || r.status === 'cancellation_pending') &&
	...
	<div class="grid">
			...
			<div class="card chart-card" style="grid-column: span 4;">
				...
				<div class="card-header">
					...
					{#if d.title.toLowerCase().includes('annual')}
						<div class="cf-top">
							<span
								>Carry forward: {formatCF(d.carryForward)}/{formatCF(
									user?.carry_forward_original ??
										user?.carry_forward_balance_original ??
										user?.carry_forward ??
										0
								)}</span
							>
							...
							<div class="cf-tip-wrap">
								...
								<span class="tooltip">
									Carry-forward is capped at 7 days and expires by 30th of April.
								</span>
							</div>
						</div>
					{/if}
				...
				<div
					class="donut fancy"
					style="--size:110px; --spent:{pct(
						d.spent,
						d.total
					)}; --spent-color: var(--spentTeal); --rest-color: var(--restSlate);"
				></div>
				<div class="legend-row">
					<div class="legend-item"><span class="chip spent"></span><span>Spent Leave</span></div>
					<div class="legend-item">
						<span class="chip unspent"></span><span>Balance Leave</span>
					</div>
				</div>
				<div class="total-line bold">
					Allocated: {d.total} | Spent: {d.spent} |
					{#if d.pending > 0}
						Pending: {d.pending} |
					{/if}
					Balance: {d.remaining}
			...
			{:else}
				<div class="calendar calendar-small">
					<div class="month">
						...
						<div class="nav">
							<button class="nav-btn" on:click={goToday} aria-label="Go to current month"
								>Today</button
							>
							<button
								class="nav-btn"
						...
						{#each days as d (d.key)}
							<button
								...
								on:click={() => {
									...
									if (d.weekend) {
										showToast('You cannot apply for leave on weekends.', 'warning', 'Weekend');
										return;
									}
								...
								title={d.limitMessage ? d.limitMessage : d.title}
							>
								{d.label}
							</button>
						{/each}
		...
		<div class="card recent-card small-card" style="grid-column: span 8;">
			...
			<div class="recent-wrap">
				{#if recent.length === 0}
					<div class="recent-empty">No leave requests application</div>
				{:else}
					{#each recent as r}
...
<dialog bind:this={modal} class="leave-modal" aria-labelledby="leave-title">
	<form class="leave-form" on:submit={submitLeave}>
		<button type="button" class="close-btn" on:click={() => modal.close()} aria-label="Close"
			>✕</button
		>
		...
		<label>
			...
			<select name="type" bind:value={leaveType} required>
				...
				{#if !user?.department
					...
					.map((d) => d.trim().toLowerCase())
					.includes('technical data - consultant')}
					<option value="UNPAID" disabled={availableAnnualLeave > 0}>
						{availableAnnualLeave > 0 ? `Unpaid Leave (Use Annual Leave first)` : `Unpaid Leave`}
					</option>
				{/if}
			</select>
		</label>
		...
		<div class="duration">
			...
			<label
				><input type="radio" name="duration" value="Full" bind:group={duration} /> Full Day</label
			>
			<label
				><input type="radio" name="duration" value="Half" bind:group={duration} /> Half Day</label
			>
		</div>
...
<style>
	...
	.donut-title {
		font-size: var(--fs-section-heading, 16px);
		font-weight: 600;
	...
	.chip.spent {
		background: var(--spentTeal);
	}
	.chip.unspent {
		background: var(--restSlate);
	}
	.total-line {
		text-align: center;
		font-size: 12px;
	...
	.leave-form input[required]:invalid,
	.leave-form textarea[required]:invalid {
		border-color: #dc2626;
	}
