...
import { sendWelcomeEmail, sendAdminNotification, sendEmailCorrection } from '../utils/emailTemplates.js';
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // EMAIL TO ADMIN
    try {
      const adminEmail = process.env.ADMIN_NOTIFICATION_EMAIL || "hr@eftech.com.my";
...
router.put("/:staff_id", async (req, res) => {
  ...
  try {
    ...
    if (emailChanged) {
      try {
        await sendEmailCorrection(email, {
          name: full_name || oldName,
          newEmail: email,
        });
      } catch (emailErr) {
        console.error("Failed to send email correction:", emailErr.message);
      }
    }
